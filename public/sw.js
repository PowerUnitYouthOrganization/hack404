...
self.addEventListener('notificationclick', function(event) {
    ...
    const targetUrl = event.notification.data?.url || 'https://hack404.dev/launchpad';
  ...
  if (event.data && event.data.type === 'mockPush') {
    try {
      ...
      if (listeners && listeners.length) {
        ...
        event.ports[0].postMessage({ status: 'success', message: 'Mock push processed' });
      } else {
        ...
        if (!self.__originalAddEventListener) {
          ...
          self.addEventListener = function(type, listener, options) {
            ...
            return self.__originalAddEventListener.call(this, type, listener, options);
          };
        }
        ...
        try {
          ...
          if (handler) {
            ...
            event.ports[0].postMessage({ status: 'error', message: 'No push event handler found' });
          }
        } catch (innerError) {
          event.ports[0].postMessage({ status: 'error', message: innerError.toString() });
        }
      }
    } catch (error) {
      event.ports[0].postMessage({ status: 'error', message: error.toString() });
