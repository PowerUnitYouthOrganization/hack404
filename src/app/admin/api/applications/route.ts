...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        const reviewedApplicationsResult = await db
            .select({ count: count() })
            .from(applications)
            .where(eq(applications.applicationReviewed, true));
        ...
        const recentApplications = await db
            .select({
                ...
                reviewed: applications.applicationReviewed,
        ...
        const response = {
            ...
            pendingReview: submittedApplicationsResult[0].count - reviewedApplicationsResult[0].count,
            streams: {
